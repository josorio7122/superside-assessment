...
| B6 | WorkOS User Management widget (`<UsersManagement />`) for in-app team admin | User mgmt |
...
| B12 | Tenant onboarding via WorkOS hosted sign-up | Multi-tenant |
...
| C3 | LLM gateway via OpenRouter (text/multimodal) + OpenAI direct (image) wrapped in `packages/ai`: provider routing, retries with circuit breaker, prompt cache | Reliability |
...
| C9 | Analytics scaling decision (skip rollups / build daily aggregate table / migrate to ClickHouse), gated on Beta-period metrics at week 6 | Analytics scale |
...
- **Weeks 6–8 (Final Rollout, many users)**: B8 quality-of-life filters + search; C9 analytics scaling decision (week-6 audit picks skip / aggregates / ClickHouse based on Beta volume). Security review, performance pass, eval threshold tightening, on-call rotation.

## What's deliberately not in scope

- **Pre-committed analytics scaling.** The week-6 audit gate is *the* scope decision: skip rollups (low volume), build daily aggregates (medium), migrate to ClickHouse (high volume or customer-facing analytics in scope). We don't pre-commit; Beta numbers drive it.
- **Image-to-image at MVP.** OpenAI's `images.edit` does not currently accept `gpt-image-2`. Image-to-image moves to Beta via fal.ai's `gpt-image-2/edit`, paired with the fal.ai integration that also brings image fallback.
- **Per-tenant rate limits and concurrency caps.** Same-provider retries plus single-key concurrency are sufficient until customer usage demands tighter caps. Adding caps is a config change in `packages/ai`.
